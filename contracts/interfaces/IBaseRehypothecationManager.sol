...
interface IBaseRehypothecationManager {
	function lend(address asset, address collateral, uint assetAmount, uint collateralAmount) external;
	function withdraw(address asset, address collateral, uint amount) external;
	function borrow(address asset, address collateral, uint assetAmount, uint collateralAmount) external;
