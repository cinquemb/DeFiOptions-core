...
import "../../interfaces/IInternalHedgingManagerFactory.sol";
import "../../interfaces/IUnderlyingCreditProvider.sol";
import "../../utils/Convert.sol";


contract InternalHedgingManager is BaseHedgingManager {
    address private internalRehypothecationAddr;
    address private internalHedgingManagerFactoryAddr;
    ...
    constructor(address _deployAddr, address _poolAddr) public {
        ...
        internalHedgingManagerFactoryAddr = deployer.getContractAddress("InternalHedgingManagerFactory");
        address _internalRehypothecationAddr = IInternalHedgingManagerFactory(internalHedgingManagerFactoryAddr).getRemoteContractAddresses();
        vault = IUnderlyingVault(deployer.getContractAddress("UnderlyingVault"));        
        require(_internalRehypothecationAddr != address(0), "bad order book");
    ...
    function getPosSize(address underlying, bool isLong) override public view returns (uint[] memory) {
        ...
        if (isLong == true) {
            ...
            posSize[0] = IBaseRehypothecationManager(internalRehypothecationAddr).notionalExposure(address(this), address(exchange), udlCdtk);
        } else {
            //(collateral == exchange balance, asset == udl credit)
            posSize[0] = IBaseRehypothecationManager(internalRehypothecationAddr).notionalExposure(address(this), udlCdtk, address(exchange));
        }
    ...
    function getHedgeExposure(address underlying) override public view returns (int256) {
        ...
        //(asset == exchange balance, collateral == udl credit)
        totalExposure = totalExposure.add(int256(IBaseRehypothecationManager(internalRehypothecationAddr).notionalExposure(address(this), address(exchange), udlCdtk)));
        //(collateral == exchange balance, asset == udl credit)
        totalExposure = totalExposure.sub(int256(IBaseRehypothecationManager(internalRehypothecationAddr).notionalExposure(address(this), udlCdtk, address(exchange))));
            ...
            if (exData.real > 0) {
                ...
                for(uint i=0; i< exData.openPos.length; i++){
                    if (exData.openPos[i] != 0) {
                        ...
                        if (IERC20_2(exData.udlCdtk).allowance(address(this), internalRehypothecationAddr) > 0) {
                            IERC20_2(exData.udlCdtk).safeApprove(internalRehypothecationAddr, 0);
                        }
                        IERC20_2(exData.udlCdtk).safeApprove(
                            internalRehypothecationAddr, 
                            IBaseRehypothecationManager(internalRehypothecationAddr).borrowExposure(address(this), address(exchange), exData.udlCdtk)
                        );

                        IBaseRehypothecationManager(internalRehypothecationAddr).repay(address(exchange), exData.udlCdtk, udlFeedAddr);
                    }
                }
            ...
            if (exData.real < 0) {
                ...
                for(uint i=0; i< exData.openPos.length; i++){
                    if (exData.openPos[i] != 0) {
                        ...
                        if (IERC20_2(address(exchange)).allowance(address(this), internalRehypothecationAddr) > 0) {
                            IERC20_2(address(exchange)).safeApprove(internalRehypothecationAddr, 0);
                        }
                        IERC20_2(address(exchange)).safeApprove(
                            internalRehypothecationAddr, 
                            IBaseRehypothecationManager(internalRehypothecationAddr).borrowExposure(address(this), exData.udlCdtk, address(exchange))
                        );
                        IBaseRehypothecationManager(internalRehypothecationAddr).repay(exData.udlCdtk, address(exchange), udlFeedAddr);
                    }
                }
            ...
            if (exData.pos_size != 0) {
                ...
                for (uint i=0; i< exData.allowedTokens.length; i++) {
                    if (exData.totalPosValueToTransfer > 0) {
                        ...
                        if (exData.t.allowance(address(this), internalRehypothecationAddr) > 0) {
                            exData.t.safeApprove(internalRehypothecationAddr, 0);
                        }
                        exData.t.safeApprove(internalRehypothecationAddr, v);
                        ...
                        //(collateral == exchange balance, asset == udl credit), short
                        IBaseRehypothecationManager(internalRehypothecationAddr).lend(exData.udlCdtk, address(exchange), exData.pos_size, v, udlFeedAddr);
                        IBaseRehypothecationManager(internalRehypothecationAddr).borrow(exData.udlCdtk, address(exchange), exData.pos_size, v, udlFeedAddr);
            ...
            if (exData.pos_size != 0) {
                ...
                for (uint i=0; i< exData.allowedTokens.length; i++) {
                    if (exData.totalPosValueToTransfer > 0) {
                        ...
                        //approve collateral && lend && borrow
                        IBaseRehypothecationManager(internalRehypothecationAddr).lend(address(exchange), exData.udlCdtk, exData.totalPosValue, exData.pos_size.div(exData.poolLeverage), udlFeedAddr);
                        IBaseRehypothecationManager(internalRehypothecationAddr).borrow(address(exchange), exData.udlCdtk, exData.totalPosValue, exData.pos_size.div(exData.poolLeverage), udlFeedAddr);
